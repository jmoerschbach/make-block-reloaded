...
static const uint8_t audio_on_icon[] PROGMEM = { 0x0c, 0x0c, 0x1e, 0x3f, 0x3f,
		0x00, 0x12, 0x0c, 0x21, 0x1e };

static const uint8_t audio_off_icon[] PROGMEM = { 0x0c, 0x0c, 0x1e, 0x3f, 0x3f };

static const uint8_t brightness_icon[] PROGMEM = { 0x08, 0x1c, 0x00, 0x08, 0x2a,
		0x1c, 0x7f, 0x1c, 0x2a, 0x08, 0x00, 0x1c, 0x08 };

static const uint8_t ok_icon[] PROGMEM = { 0x0e, 0x11, 0x11, 0x11, 0x0e, 0x00,
		0x1f, 0x04, 0x0a, 0x11 };
...
void config_draw_menu(uint8_t sel) {
	...
	if (config_audio)
		config_draw_icon((W - sizeof(audio_on_icon)) / 2, 19, audio_on_icon,
				sizeof(audio_on_icon), 6, sel == 0, -1);
	else
		config_draw_icon((W - sizeof(audio_off_icon)) / 2, 19, audio_off_icon,
				sizeof(audio_off_icon), 6, sel == 0, -1);

	config_draw_icon((W - sizeof(brightness_icon)) / 2, 12, brightness_icon,
			sizeof(brightness_icon), 7, sel == 1,
			((uint16_t) config_brightness * 14) / 255);

	config_draw_icon((W - sizeof(ok_icon)) / 2, 4, ok_icon, sizeof(ok_icon), 5,
			sel == 2, -1);
}
