...
uint8_t initials_process() {
	...
	if ((wasLeftPressed()) && initials_cur_chr > 0) {
	...
	if (((wasRotatePressed()) || wasRightPressed())
			&& (initials_cur_chr < 14)) {
	...
	// "drop" is actually up
	if (wasDropPressed() || wasDownPressed()) {
		...
		if (wasDropPressed()) {
			...
			else if ((c == ' ') && (initials_cur_chr == 0))
				c = 'A';
			else if (c == ' ')
				c = INITIALS_BACK;
			else if (c == INITIALS_BACK)
				c = INITIALS_DONE;
			else if (c == INITIALS_DONE)
				c = 'A';
		} else {
			if ((c == 'A') && (initials_cur_chr == 0))
				c = ' ';
			else if (c == 'A')
				c = INITIALS_DONE;
			else if ((c > 'A') && (c <= 'Z'))
